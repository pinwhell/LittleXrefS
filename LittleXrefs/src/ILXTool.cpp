...
{
    RefsEngine = new ArmReferenceEngine(GetCapstoneManager());
}
...
void ILXTool::Run()
{
    while (true)
    {
        ...
        if (typeName == "_clazz")
        {
            ...
            FindClazzRefs(clazz, funcRefsLists);
        }
            ...
            FindClazzRefs(clazz, clazzOff, funcRefsLists);
        }
        else {
...
{
    RefsEngine = new Arm64ReferenceEngine(GetCapstoneManager());
}
